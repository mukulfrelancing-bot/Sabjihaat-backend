...
const GITHUB_EMAIL = process.env.GITHUB_EMAIL || 'mukulfrelancing@gmail.com';
const GITHUB_DB_FILE = 'sabjihaat.db';
...
function initDatabase() {
    ...
    // First try to restore from GitHub
    restoreDatabaseFromGitHub().then(restored => {
        if (restored) {
            console.log('✅ Database restored from GitHub');
        } else {
            console.log('⚠️ Using new database');
            setupNewDatabase();
        }
        
        // Start auto-backup to GitHub
        startAutoBackup();
        
        // Auto seed products if needed
...
function setupNewDatabase() {
    db = sqlite3('sabjihaat.db', { verbose: console.log });
...
async function backupDatabaseToGitHub() {
    try {
        ...
        // Read database file
        const dbContent = fs.readFileSync('sabjihaat.db');
        ...
        const options = {
            ...
            path: `/repos/${GITHUB_REPO}/contents/${GITHUB_DB_FILE}`,
            method: 'PUT',
            headers: {
        ...
        const response = await makeGitHubRequest(options, requestData);
        
        if (response.statusCode === 201 || response.statusCode === 200) {
            console.log('✅ Database backed up to GitHub successfully');
            return true;
        } else {
            console.error('❌ GitHub backup failed:', response.statusCode, response.body);
            return false;
        }
...
async function restoreDatabaseFromGitHub() {
    try {
        ...
        const options = {
            hostname: 'api.github.com',
            path: `/repos/${GITHUB_REPO}/contents/${GITHUB_DB_FILE}?ref=${GITHUB_BRANCH}`,
            method: 'GET',
            headers: {
                'Authorization': `token ${GITHUB_TOKEN}`,
                'User-Agent': 'Node.js',
        ...
        if (response.statusCode === 200) {
            ...
            // Save to local file
            fs.writeFileSync('sabjihaat.db', fileContent);
            ...
            console.log('⚠️ No database found on GitHub, will create new one');
            return false;
        }
...
// server.js में autoSeedProductsIfNeeded function
function autoSeedProductsIfNeeded() {
    try {
        ...
        // seed-products.js से products import करें
        ...
        console.log("✅ Auto seed completed - Added " + seedProducts.length + " products");
    } catch (error) {
...
// Manual GitHub backup trigger (Admin only)
app.post('/api/admin/backup', authenticateToken, async (req, res) => {
    try {
        const success = await backupDatabaseToGitHub();
        
        if (success) {
            res.json({ 
                success: true, 
                message: 'Database backed up to GitHub successfully' 
            });
        } else {
            res.status(500).json({ 
                success: false, 
                error: 'Failed to backup to GitHub' 
            });
        }
    } catch (error) {
...
app.listen(PORT, () => {
    ...
    // Initialize database (will try to restore from GitHub first)
    initDatabase();
});
...
process.on('SIGTERM', () => {
    ...
    // Backup to GitHub before shutdown
    backupDatabaseToGitHub().then(() => {
