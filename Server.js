...
const productSchema = new mongoose.Schema({
    ...
    cloudinaryId: { type: String }, // Store Cloudinary ID for deletion
...
app.get('/api/products', async (req, res) => {
    try {
        ...
        const products = await Product.find(query)
            .sort({ 
                stock: -1, // In-stock products first
                category: 1,
                name: 1 
            })
            .select('-__v');
    ...
    try {
        ...
        if (image && image !== existingProduct.image && image.startsWith('data:image')) {
            ...
            if (existingProduct.cloudinaryId) {
                await deleteFromCloudinary(existingProduct.cloudinaryId);
                cloudinaryId = null;
                imageUrl = 'https://images.unsplash.com/photo-1542838132-92c53300491e?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80'; // Default image
            }
        }
...
app.post('/api/upload-cloudinary', authenticateToken, async (req, res) => {
    try {
        ...
        console.log('📤 Direct Cloudinary upload request...');
...
app.get('/', (req, res) => {
    res.send(`
        ...
        <body>
            <div class="container">
                <h1>
                    <span>🚀 Sabji Haat Backend API</span>
                </h1>
                ...
                <div class="endpoint">
                    <span class="method put">PUT</span> <code>/api/products/:id</code>
                    <p>Update product (Admin only - requires token)</p>
                </div>
                
                <div class="endpoint">
                    <span class="method delete">DELETE</span> <code>/api/products/:id</code>
                    <p>Delete product (Admin only - requires token)</p>
                </div>
                ...
                <h3 style="margin-top: 30px; color: #666;">🔧 Database Status: 
            ...
            <script >// Fetch stats on page load
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (productsData.success) {
                                document.getElementById('productCount').textContent = productsData.count;
                            }
                        } catch (e) {
